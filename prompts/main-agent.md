...
2. When analyzing agent output, cross-reference with History to judge whether results are reasonable.
...
5. For `error` signals, leverage History for prior experience with similar errors.
6. For complex or high-risk tasks, consider using `/opsx:` commands to guide the agent.
7. When generating prompts for `[TASK_READY]`, be specific: reference files, functions, patterns, and constraints relevant to the task. Include context from completed tasks.
